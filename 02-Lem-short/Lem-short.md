...
**Lem** はほぼ全ての機能がCommon Lispにより実装されているテキストエディタです。
Common Lispの開発環境がデフォルトで組込まれており、また、Lem自体がCommon Lispにより拡張可能です。

本書の執筆陣の多くがCommon Lispでの開発にLemを使用しており、Common Lispの開発環境として十分な実績があります。
また、Common Lisp以外のプログラミング言語サポートも徐々に追加されています。
デフォルトの操作体系はEmacsと似たものになっていますが、vi (Vim)モードも用意されています。

本章では、3章以降で必要とされるLemの基本操作、Common Lisp開発に関連した機能を紹介します。
Lemの用語や拡張機能の実装方法などの、より詳細な解説については7章 「詳解Lem」を参照してください。
...
Lemは256色表示をサポートしたターミナル上で使われることを想定しています。
Linuxのターミナルの場合、デフォルトでは8色しか表示できない事が多いので、以下のように明示的に`TERM`環境変数で明示的に256色を指定しておきます。
...
初回起動時はLemのビルドを行うので多少時間がかかりますが、次回以降はコンパイル済みのLispイメージをロードするだけなので一瞬で起動するようになります。

lemのビルドのためには ncurses というライブラリが必要です。インストールされておらずlemのビルドが失敗する場合は以下のようにインストールしてください。
...
- 右端の`All`はファイル全体に対する相対位置を表しています
  ...
  - `Top`または`Bot`はスクロールされているファイルの先頭または末尾が表示されていることを表しています
  - `50%`のように割合が表示されているときは、スクロールされているファイル全体の50%の位置に表示領域がフォーカスされていることを表しています
...
`M-x`は少し特殊な動きをします。`M-x`を入力した時点で画面中央にプロンプトが現われ、続いてコマンドを入力し、`Return`で確定することで任意のコマンドを実行できます。コマンド名は途中まで入力した状態で`Tab`を押すことで補完できます。
コマンド名を明に入力するこの一連の操作は、`M-x describe-bindings`のように表記されます。
...
この入力画面でも`Tab`を押すと補完が働き、既存のディレクトリやファイルから選択できます。また、存在しないファイル名を入力すると新規にファイルが作られます。
