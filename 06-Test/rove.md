...
Common Lispには数多くのテストライブラリがありますが、最も多く使われているものはFiveAMとProveです。

FiveAMはシンプルなテストライブラリで、Common Lispライブラリで古くから使われています。Lispのインタラクティブな環境での実行を念頭に置いたという

一方でProveはテスト結果の色付けやコマンドラインインターフェイスなどテスト実行環境周辺にも目を向けて作られています。
...
この本ではRove 0.9.1を前提としています。各環境のRoveのバージョン番号を確認し、古いバージョンの場合はQuicklispのdistをアップデートしてください。
