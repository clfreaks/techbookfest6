...
Dexadorの最も基本的かつ重要な関数は@<tt>{dex:get}で、GETメソッドで指定のURLからデータを取得する。次のコードは@<tt>{http://lisp.org/}にアクセスし、そこから取得したHTMLデータを文字列として@<tt>{*html-source*}に設定する。
...
@<tt>{get}関数は(1) 本体データの文字列、(2) HTTPステータスコード、(3) レスポンスヘッダーのハッシュテーブル、(4) URI構造体、(5) 読み出し元ソケットのストリームの5つを多値で返す。次は上で実行した@<tt>{get}関数の全ての返り値を列挙したものである。
...
ここで紹介するPlumpと次節で紹介するCLSSは同じ作者のプロダクトで、セットで使われる。これらはDexadorと同様にQuicklispからインストールできる。以下のように、@<tt>{ql:quickload}には複数のパッケージ名をリストとして与えることもできる。
...
例えば以下のようにして、前述のDexadorの@<tt>{get}の結果をパースすると、ルートノードのオブジェクトが返る。
...
各ノードの子ノードのベクタは@<tt>{plump:children}で、親ノードは@<tt>{plump:parent}でそれぞれ得られる。
...
また、DOMツリーを走査し、各ノードに対して関数を適用する高階関数@<tt>{plump:traverse}が用意されている。
...
@<tt>{traverse}にはDOMツリーの各ノードに適用される関数を渡す。キーワードパラメータ@<tt>{:test}に述語関数を渡すと、それを満足するノードに対してのみ関数が適用される。ここでは述語関数@<tt>{plump:text-node-p}を渡しているので、テキストデータを持つノードに対してのみ関数が適用され、そのノードの持つ文字列をリストに@<tt>{push}していっている。最後に@<tt>{nreverse}で順序を反転させ、@<tt>{concatenate}で連結して返すという関数だ。
...
同様にして、取得したDOMツリーに対して変更を加えることもできる。例えばテキストノードの文字列を全て大文字に変更するには、@<tt>{traverse}でDOMツリーを走査しながら、@<tt>{text-node-p}を満足するノードオブジェクトのスロットに@<tt>{setf}で新しい値を設定すればよい。
...
例えばIMGタグでルートノード以下を検索すると、Plumpのノードのベクタが得られる。その最初の要素を変数@<tt>{*img-node*}に持っておく。
...
このノードを@<tt>{describe}してみると、子ノードを持っておらず、タグ名はIMGで、属性の情報をハッシュテーブルとして持っていることが分かる。
...
なお、@<tt>{plump:attributes}でノードオブジェクトから属性のハッシュテーブルを取得でき、各属性値には@<tt>{plump:attribute}でアクセスできる。このハッシュテーブルの中身を表示してみると、属性値としてsrcとaltを持っていることが分かる。
...
このサイトをブラウザのインスペクタで調べると、内容のまとまりで@<tt>{w-section}というクラスが付いていることが分かる。CLSSを使えば、このクラス名でDOMツリーを検索することができる。なお、CSSと同様に、クラスの場合はクラス名の前に@<tt>{.}を付け、IDの場合はID名の前に@<tt>{#}を付ける。また、タグとの組み合わせで指定することもできる。
...
ここまでに説明したことを用いれば、様々なサイトから情報を取り出すことができる。@<tt>{dex:get}でサイトのURLからHTMLを文字列として取得し、@<tt>{plump:parse}でCLOSオブジェクトのノードから成るDOMツリーを作り、@<tt>{clss:select}で所望のノードを取り出す。その後は@<tt>{concat-node-text}や@<tt>{plump:attribute}などで必要な値を取り出せばよい。



Chromeの場合、インスペクタのHTMLソースから、クラスやID、タグといったものを調べられる。また、HTMLソースの要素上で右クリックして「Copy > Copy selector」を選択することで、その要素に対応するCSSセレクタをクリップボードにコピーできる。これをそのまま@<tt>{clss:select}に渡せば該当部分に対応するノードを得られる。



Chromeにはインスペクタの他にもHTTPリクエストとそのレスポンスを記録する機能などもあり、それらはDevToolsという画面にまとまっている。DevToolsは右上のメニューから「More tools > Developer tools」を選択するか、キーボードから@<tt>{Ctrl + Shift + I}を入力することで開くことができる。


=== ログインを必要とするサイト


さて、ここまでのところはデータの取得は@<tt>{dex:get}のみで事足りていたが、ログインを必要とするサイトではフォームからログイン情報を送信し、さらにその後のセッション管理が必要になるため、やや複雑になる。



Dexadorには@<tt>{dex:get}の他にも、@<tt>{dex:post}や@<tt>{dex:request}などのリクエスト関数があり、ほとんど同じように使える。これらのリクエスト関数にはCookieオブジェクトを渡すことができ、これを使ってログイン状態を保持できる。
全体の流れとしては、まず空のCookieオブジェクトを生成し、それを@<tt>{dex:post}に渡してフォームをサーバに送信することでCookieに値が設定される。以降では同じCookieオブジェクトを@<tt>{dex:get}などに渡すとログインした状態でHTMLデータが取得できるようになる。



ここでは例として、はてなブックマークの「マイホットエントリー」のページを取得することを考える。これははてなIDに連携されたSNSなどの情報を元に、各ユーザに合わせたおすすめ記事を表示するページであり、ログインした状態でないと見られない。したがって、Cookieのない状態で、以下のはてなID@<tt>{(masatoi})のマイホットエントリーページを取得してみると、アクセス権限が無いというエラーメッセージが返る。
...
まず、ブラウザではてなのログインページ(@<tt>{https://www.hatena.ne.jp/login})を開き、DevToolsからNetworkタブを開く。この画面にはHTTPリクエストやそれに対するレスポンスが記録される。なお、ログイン成功時には、はてなのトップページへリダイレクトされるので、ページの遷移や再読み込みでログが消えないように「Preserve log」チェックボックスを選択しておく必要がある。
DevToolsのNetworkタブを開いた状態で、Webページ上のログインフォームにログイン情報を入力して送信すると、たくさんのログが表示されるので、ここからログイン情報を送っているPOSTリクエストを探す。左上の検索ウインドウからメソッドを指定してリクエストを検索することができるので、@<tt>{method:POST}と入力するとPOSTリクエストが検索できる。
...
ログインと同様に、Webサイトからログアウトするときもログアウト用のURLにPOSTリクエストを送るだけである。以下のように、ログインして一連の処理を行い、最後に必ずログアウトするようなマクロ@<tt>{with-hatena}を考えることができる。
...
このマクロ内で生成されるCookieオブジェクトは局所変数に束縛され、本体部分で使用することができる。@<tt>{with-hatena}を使うことで、ログイン/ログアウト処理を意識することなくログインが必要なURLにアクセスすることができる。
...
cl-igo@<fn>{362459e05c60f4343ce7be949d5fa564}はCommon Lispから使える形態素解析器で、MeCabの辞書を変換したバイナリ辞書を使う。辞書の変換にはJavaを使用するが、その後の推論部分はCommon Lispのみで実装されている。
...
以下のURLからMeCabの辞書データ(@<tt>{mecab-ipadic-2.7.0-20070801.tgz})と、Igoのjarファイル(@<tt>{igo-0.4.5.jar})をダウンロードする。
...
ここでダウンロードした2つのファイルと同じディレクトリで次のコマンドを実行すると、@<tt>{~/igo/ipadic}というディレクトリに辞書ができる。
...
最後に、LispのREPLなどで@<tt>{ql:quickload}でcl-igoをロードし、@<tt>{igo:load-tagger}で先程作ったバイナリ辞書のディレクトリを指定して読み込む。
以降は@<tt>{igo:parse}関数に日本語の文を与えればそれを形態素解析した結果のリストが返る。
...
ここでは説明のために、クリエイティブコモンズライセンスで公開されているLivedoorニュースコーパス@<fn>{4f5a1a80dde58a84dd94cd0d038c5891}を使用する。



このデータセットは9つのニュースサイトから合計7367個の記事を集めたものであり、出典ごとにディレクトリに分けて、1記事1ファイルのテキストファイルとして収録されている。以降では、ここでダウンロードしたファイルを@<tt>{~/datasets/livedoor}に展開したものとして話を進める。
...
テキストデータを形態素解析し、TF-IDFで単語の重要度を計算し、スパースベクトルのリストとして返す一連の処理を行うライブラリとして@<tt>{cl-docclass}がある。これはRoswellからインストールできる。
...
以下のコードで、先程ダウンロードしたLivedoorニュースコーパスのテキストデータから、機械学習ライブラリへの入力となるリスト@<tt>{*text-sparse-vectors*}が作られる。
...
@<tt>{*text-sparse-vectors*}はスパースベクトルのリストで、その長さは文書数だけあるので、7367個ある。総単語数は@<tt>{*word-hash*}から、18372個であることが分かる。1つの文書に対応するスパースベクトルを表示してみると、実際には114個の単語しか出現していないことが分かる。
...
代表的な2値分類器の組合せ方には@<tt>{1-vs-1}と@<tt>{1-vs-Rest}がある。@<tt>{1-vs-1}の方が一般に精度が良いが、クラス数の2乗に比例する計算コストがかかる。一方で@<tt>{1-vs-Rest}はクラス数に比例する計算コストで済む。ここでは@<tt>{1-vs-Rest}を用いることにする。


==== 学習器のオブジェクトを作る


学習器を作るには、@<tt>{clol:make-one-vs-rest}関数を使う。この関数には入力データの次元数(スパースベクトルの本来のサイズ)と、クラス数、学習アルゴリズムの種類を表すシンボル、学習アルゴリズムのメタパラメータを与える。
...
テストには@<tt>{clol:test}を使う。訓練データに対する予測の正解率は100%近くになるが、未知のデータであるテストデータに対しては95%程度であることが分かる。
...
1つのデータに対するクラスラベルを予測するには、@<tt>{clol:one-vs-rest-predict}を使う。
