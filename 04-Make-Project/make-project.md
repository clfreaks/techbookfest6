...
`packages.lisp`では、次のように、パッケージとエクスポートされるシンボルが定義されます。
...
この手法は、ライブラリの規模が大きくなるにつれて、パッケージ管理が難しくなります。複数のパッケージの依存関係を管理するためには互いの依存関係を把握する必要があり、依存関係の把握が難しくなります。この問題を解決するために、`one package per file`(1ファイルにつき1パッケージ)と名づけられた手法が提唱されました。
