...
ターミナル上で`lem`コマンドにファイル名を引数として与えると、Lemが起動し、そのファイルを開いて編集することができます。もしファイルが存在しない場合は、新規にファイルが作成されます。また、引数にディレクトリを指定することで、ファイラ(dired)を開いた状態でLemを起動することができます。

```
# ファイル名を指定して開く、または新規作成
$ lem <ファイル名>
