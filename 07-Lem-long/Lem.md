...
lisp-modeについての機能を一通り紹介したあと、Lemでの拡張機能の書き方をシンプルなreddit[^reddit]のビューアーを作りながら説明していきます。
...
モードではそのモードの名前や対応するキーバインド、シンタックステーブルなどを設定します。

モードにはメジャーモードとマイナーモード、グローバルモードがあり、メジャーモードは一つのバッファに一つだけ設定でき、
マイナーモードは一つのバッファに複数設定できます。
グローバルモードはエディタに一つだけ設定でき、デフォルトではemacs-modeとvi-modeがあります。

## 初期化ファイルについて

Lemの起動時に初期化ファイルが読み込まれます。
ファイル名は `~/.lemrc` または `~/.lem/init.lisp` です。以下のように書きます。
...
現在のモードのコマンド一覧は `M-x describe-bindings` で表示されます。
...
起動時に自動でvi-modeにするには `~/.lem/init.lisp` に次の式を加えます。
...
中断すると割り込みエラーでデバッガが表示されます。デバッガの使い方については後述します。
...
例えばcl-ppcreに `register-groups-bind` というマクロがありますが、これはcl-ppcreを読み込んでない状態では関数と同じインデントにされてしまいます。
...
![コンパイル](../images/02-lem-compile.png)
...
例としてHyperSpecのmacroexpandの例で定義されているマクロ `alpha`、`beta` でマクロ展開してみます。
...
SLIMEではエディタからaproposを使える以下のインターフェースを用意しています。
...
![Apropos](../images/02-lem-apropos.png)
...
![inspect](../images/02-lem-inspect-1.png)
...
![inspect 2](../images/02-lem-inspect-2.png)
...
SLIMEでのデバッガはSLDBと呼ばれています。評価した式でエラーが起こった場合はSLDBが起動し、専用のsldbバッファが表示されます。

次の画像はREPLでエラーが出る式を評価してSLDBが出た例です。

![SLDB](../images/02-lem-sldb-1.png)

alexandria:lastcarは引数にリストを期待していますが文字列を渡しているので型エラーが出ています。

sldbバッファの内容を一つずつ見ていきます。

まず最初に表示されている次のメッセージは、エラーメッセージとコンディションの型です。
...
その後に続く次のメッセージは、エラー後にどういう対応をするかをユーザーに尋ねています。
...
最後にBacktrace以下は、フレームのリストを表示しています。
...
実際にsldbを使ってみましょう。まずこのようなファイルがあるとします。
...
![sldb practice 1](../images/02-lem-sldb-practice-1.png)
...
![sldb practice 2](../images/02-lem-sldb-practice-2.png)
...
![sldb practice 3](../images/02-lem-sldb-practice-3.png)

sldbからでもinspectを使うことが出来ます。新しく追加した関数をmatcherオブジェクトのactionにしてしまいましょう。match関数内に対応する0から数えて2のフレームを選択し、詳細が表示されるので、引数のmatcherを選択するとinspectが起動します。

![sldb practice 4](../images/02-lem-sldb-practice-4.png)
...
![sldb practice 5](../images/02-lem-sldb-practice-5.png)
...
- `https://github.com/clfreaks/lem-posts-list`

完成は次のようになります。

![完成図](../images/02-extension-preview.png)
...
`kind` が `:left-inserting` ならポイントと同じ位置に文字列を挿入したときに右にずれ、`:right-inserting` ならそのままです。

`kind` が `:temporary` の場合は何も行いません。

`kind` を指定しなければ渡された `point` と同じ値になります。

`kind` が `:temporary` 以外ならpointをbufferが保持しておく必要があるので不要になったら明示的に削除しなければいけません。
...
`with-point` の `kind` を省略した場合は `:temporary` になります。
...
本体に次のように書くことで、モードが有効になったときに現在のバッファを読み込み専用になるようにします。
...
[^reddit]: 米国のソーシャルニュースサイト、掲示板。https://www.reddit.com
