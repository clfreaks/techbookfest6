...
また書き込んだ範囲に:postプロパティを設定してあり、あとからその行の投稿オブジェクトを参照できるようにしています。
...
bufferのread-onlyフラグを引数の値に変更した後body内を実行し、終わったら元に戻します。
...
define-commandでコマンドを定義するとM-xで呼び出せるようになり、キーにも束縛できるようになります。
